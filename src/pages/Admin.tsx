...
export const AdminPage: React.FC = () => {
  ...
  const [profile, setProfile] = useLocalStorage<ProfileData>(
    "talha_portfolio_profile",
    initialProfileData,
  );
  const [projects, setProjects] = useLocalStorage<Project[]>(
    "talha_portfolio_projects",
    initialProjectsData,
  );
  const [skills, setSkills] = useLocalStorage<SkillCategory[]>(
    "talha_portfolio_skills",
    initialSkillsData,
  );
  const [workflow, setWorkflow] = useLocalStorage<WorkflowStep[]>(
    "talha_portfolio_workflow",
    initialWorkflowData,
  );
  const [socials, setSocials] = useLocalStorage<SocialLink[]>(
    "talha_portfolio_socials",
    initialSocialLinks,
  );
  const [settings, setSettings] = useLocalStorage<WebsiteSettings>(
    "talha_portfolio_settings",
    initialWebsiteSettings,
  );
  const [customCV, setCustomCV] = useState<CustomCVData | null>(null);

  // Load cloud updates from Supabase if available
  useEffect(() => {
    portfolioService
      .loadFromSupabase<ProfileData>("profile")
      .then((data) => data && setProfile(data));
    portfolioService
      .loadFromSupabase<Project[]>("projects")
      .then((data) => data && setProjects(data));
    portfolioService
      .loadFromSupabase<SkillCategory[]>("skills")
      .then((data) => data && setSkills(data));
    portfolioService
      .loadFromSupabase<WorkflowStep[]>("workflow")
      .then((data) => data && setWorkflow(data));
    portfolioService
      .loadFromSupabase<SocialLink[]>("socials")
      .then((data) => data && setSocials(data));
    portfolioService
      .loadFromSupabase<WebsiteSettings>("settings")
      .then((data) => data && setSettings(data));
    portfolioService
      .loadFromSupabase<CustomCVData>("custom_cv")
      .then((data) => data && setCustomCV(data));
  }, []);
